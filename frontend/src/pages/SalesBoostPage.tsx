...
export function SalesBoostPage() {
  ...
  async function upload(file: File | undefined) {
    ...
    try {
      ...
      setFeedback(`Imported ${result.created}; skipped ${result.duplicates} duplicates and ${result.invalid_rows.length} invalid rows.`);
      await load();
    ...
    <Layout>
      ...
      <div className="boost-grid">
        <section className="data-panel form-panel"><div className="panel-heading"><div><h2>Add successful product</h2><p>Keyword and category overlap can contribute up to 20 points.</p></div></div>
          <form className="stack-form" onSubmit={submit}><label>Product title<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Wireless Neck Fan" required /></label><label>Category<input value={category} onChange={(event) => setCategory(event.target.value)} placeholder="Home & Kitchen" required /></label><label>Keywords<input value={keywords} onChange={(event) => setKeywords(event.target.value)} placeholder="cooling, portable fan, summer" /><small>Separate keywords with commas.</small></label><button className="button button-primary">Add product</button></form>
