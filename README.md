...
- **Google Trends collector** — Playwright browser navigation and capture of the timeline response generated by the page. Keywords are collected in comparison batches of at most five. The first confirmed HTTP 429 ends the run and activates a Redis cooldown so repeated clicks do not open more browser sessions.
- **Scoring engine** — category/keyword boost plus validated structured OpenAI or Gemini output, with deterministic fallback on missing key, timeout, invalid response, or provider failure. Missing trend evidence is treated as unavailable, never as zero demand.
...
The committed Compose file contains only variable references and passes provider keys solely to the Celery scoring worker at runtime. Secret values remain in the ignored local `.env`; they are never embedded in an image, frontend bundle, source file, or committed Compose configuration. Starting without keys exercises the deterministic fallback path. To verify cloud providers against a local ignored `.env`:
...
4. Select **Run trend collection** and observe its Celery task status and result counters. On a blocked IP the first HTTP 429 must finish the task, activate the cooldown, and show `No fresh trend data` plus collected/failed/rate-limited counts instead of leaving the page waiting. A second click during the cooldown must finish without launching Playwright again.
...
- Google Trends returned HTTP 429 for the current verification IP. A clean 20-product run stopped after one browser attempt, reported `0 collected / 20 failed / 20 rate-limited`, activated the configured cooldown, did not rescore from failed evidence, and did not fabricate values. Retry after the provider cooldown to obtain fresh snapshots.
